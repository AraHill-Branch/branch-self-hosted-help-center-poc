...
This article helps you configure link behavior for redirects and link domains.
...
In the left-side navigation of Branch, find the [**App Settings**](https://branch.dashboard.branch.io/configuration/general) page under **Configure**. Find the **General** tab, and in the **Required Redirects** section, enter in your fallback URL for mobile devices that do not have a specified redirect.
...
1. In Branch, navigate to [**App Settings**](https://branch.dashboard.branch.io/configuration/general) → **General** tab → **Required Redirects** section → **Android Redirects**.
...
If you choose to **Enable App Links** you need to generate a SHA256 Cert Fingerprint. Use one of the following methods to do this:
...
2. Run `keytool -list -v -keystore my-release-key.keystore` to generate a fingerprint.  
    \* Example fingerprint:  
   `AA:C9:D9:A5:E9:76:3E:51:1B:FB:35:00:06:9B:56:AC:FB:A6:28: CE:F3:D6:65:38:18:E3:9C:63:94:FB:D2:C1` to add to Branch.
...
1. In Branch, navigate to [**App Settings**](https://branch.dashboard.branch.io/configuration/general) → **General** tab → **Required Redirects** section → **iOS Redirects**.
2. If you choose to **Enable Universal Links**, you need to enter your **Apple App Prefix** which can be found within the Apple Developer Portal for your app.
3. To enable deferred deep linking via [NativeLink](https://help.branch.io/docs/nativelink-deferred-deep-linking#integration-guide)™, you can select the **Enable NativeLink™** checkbox. Doing so means your users will be presented with a [Deepview](deepviews-index.md) that lets them continue their experience in an app, even if they don’t have it installed.
...
::: danger Warning
For any user running on an iOS version below 12.3, a popup first appears prompting the user to open in the App Store when being redirected to the app store. The user must select **OK** to be fully routed to the App Store to download your app. This popup cannot be removed from the user flow as it's inherent to iOS.
:::
...
1. In Branch, navigate to [**App Settings**](https://branch.dashboard.branch.io/configuration/general) → **General** tab → **Social Media Preview** section.
2. Fill in the following fields to customize how the link to your install page looks when shared or re-shared on social media:  
    a. Link Title  
    b. Description  
    c. Thumbnail Image
...
To see your app’s default link domain and alternate link domain, navigate to the [General tab](https://dashboard.branch.io/configuration/general) of the App Settings page in Branch.

When you first create your account, you're assigned a unique domain for all of your Branch Links.
...
::: warning Caution
Before changing your Branch link domain, it is important to understand all behavior changes that occur when you do so. This helps ensure you do not run into any unwanted experiences for your users selecting your Branch Links.
:::

| Change | Example | Behavior |
| --- | --- | --- |
| `app.link` to `app.link` | `abcdef.app.link` → `mycompany.app.link` | - Your old `app.link` URLs will redirect to the **LINK NOT FOUND** page. - Your old `app.link` URLs will fail to open your mobile app. - Your new `app.link` URLs will not open your app until you update your code to utilize the new link domain ([iOS](ios-basic-integration.md) / [Android](android-basic-integration.md)). - If you are already using your original/old `app.link` URLs in the wild and need them to keep working, we recommend exploring [Using Your Own Domain](advanced-settings-configuration.md#using-your-own-domain) instead. |
...
You have the ability to change your `app.link` subdomain for your Branch Links to better match your brand. We recommend setting your `app.link` subdomain to the name of your app, in case you have multiple mobile apps under your brand.
...
2. Select the **Change My** `app.link` **Subdomain** button.
...
- Whenever you change the NS or CNAME records of your domain, you are making Branch the authoritative registrar of your domain. This grants Branch control of your domain and you lose access to that custom subdomain or custom root domain. The web page becomes blank, and control of the domain changes to Branch.
- Branch uses the domain to route all deep linked traffic.
...
- **How it works**: This configuration relies on Universal Links (iOS) and Android App Links to prioritize opening the app. If a user has your app and clicks a web link, the link opens in an in-app webview.
...
1. In Branch, select **App Settings** from the navigation menu.
