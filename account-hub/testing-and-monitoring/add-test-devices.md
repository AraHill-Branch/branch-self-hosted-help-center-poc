...
To easily test your campaign setup, we recommend you first add a test device to your Branch account. Doing so tells Branch servers that resulting conversions from said device are for testing purposes only; i.e. these conversions are not considered normal traffic and therefore not treated as such.
...
- The test device is specific to the app you add it under. If you use it to test other apps, Branch treats it like live traffic, and it's attributed and used for downstream analytics.
