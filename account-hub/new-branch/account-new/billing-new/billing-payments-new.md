---
title: "Manage payments (New)"
slug: billing-payments-new
---

The [**Billing tab**](https://branchdocs.app.link/billing-new-to-billing-settings) is where you can see your payment information and make changes if you need to.

The view you see in the **Billing** tab depends on whether you have access to Branch’s new or legacy billing experience.

## Payment information

Use the **Payment information** module to see the payment method details associated with your account.

To make changes to your payment method, select the **Update** button.

When you are finished, select **Update card**.

## FAQ

<details>
<summary>What happens if my credit card is not accepted (Error 322)?</summary>

If the system provides an Error 322 when entering your credit card, follow these troubleshooting steps:

1. Verify the billing address matches the billing address on your credit card statement.
2. Verify that there aren’t too many sales or authorizations on your card in a short period of time.
3. For international customers, verify your credit card has permission to accept US charges.
4. Verify your credit card is not a prepaid card.

If you continue to experience issues, please contact your financial institution for further assistance. If there’s any action and/or transaction Branch can help with, contact our support team at [billing@branch.io](mailto:billing@branch.io).

</details>

<details>
<summary>What payment methods do you accept?</summary>

For online subscriptions, we only accept credit cards. Please ensure your credit card meets the following criteria:

1. Credit card must be a revolving account with a line of credit.
2. The billing address entered matches the billing address in your credit card statement.
3. For international customers, make sure the credit card accepts foreign transactions.

Once your payment and billing information has been entered, no further action is required to pay off your monthly invoice.

</details>