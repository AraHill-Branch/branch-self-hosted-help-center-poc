...
Revenue data only populates for `PURCHASE` events. The event must be a standard Branch Commerce Event with the type `PURCHASE`, as outlined in the Commerce table in [Track Commerce events](track-branch-events.md#track-commerce-events).

If you use a different type of event other than a `PURCHASE` event, you'll only see the data in Liveview, and the same data does not populate in Branch.
