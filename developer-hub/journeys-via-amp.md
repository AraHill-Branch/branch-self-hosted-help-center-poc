---
title: "Journeys on Accelerated Mobile Pages (AMP)"
slug: journeys-via-amp
---
...
- Journeys uses your alternate domain for Universal Links. Make sure you include your `xxxx-alternate.app.link` domain in your [Associated Domains](https://help.branch.io/developers-hub/docs/ios-basic-integration#3-configure-associated-domains).

## Setup

1. Add the Branch AMP SDK to your site. The AMP SDK consists of 2-3 snippets that you can insert into your AMP page. Add the following snippet between the AMP page’s `<>` tags:
...
2. Modify the following snippet by replacing `DOMAIN_HERE` with your `xxxx.app.link` Branch Link domain (*do not use a custom subdomain*), and `BRANCH_KEY_HERE` with your Branch API key - in both the `amp-list` tag and the `amp-iframe` tag. You can find these in [Link Settings](https://dashboard.branch.io/link-settings) and [Account Settings](https://dashboard.branch.io/account-settings).
...
3. From there, add the modified snippet between the `<>` tags of your AMP page, preferably near or at the bottom.
4. Lastly, if you do not already have the following AMP scripts on your page, add them between the AMP page's `<>` tags:
...
::: info Include canonical URL for SEO and deep linking
[Google recommends](https://www.ampproject.org/docs/guides/discovery) that on your AMP page, you include a reference to the canonical URL of the non-AMP page with the same content. For example, you should include a tag like this in the `<>` section of your AMP page: . This helps make your AMP page discoverable, and likely helps ensure that SEO information is shared between these two pages. Additionally, Branch automatically embeds the canonical URL in the Journey link data, leading to better identification of content and the ability to use this key for deep linking.
:::
...
1. When you click **Select Template** from the **Configure Views** step, the **standard bottom banner** view type should already be selected. This is currently the only template supported on AMP because Google requires that banners not show in the top 75% of an AMP page.
2. Hover over the template and click **Create**.
3. Customize the template and click **Save & Continue** when you're done.
...
- [Reference this guide for more information on troubleshooting and error messages on Journeys.](troubleshoot-journey-app-banners.md)
...
2. Add deep link data to a Journey in the dashboard

::: info Use $canonical_url for deep linking
AMP Journeys, along with regular Journeys and the Short Link Creator, automatically embeds `$canonical_url` in your link data based on meta tags on your web or AMP page. If you use this key to route to specific content in your app, you do not have to add anything extra for AMP.
:::
...
- Event-based audience rules work within AMP only or after a Branch Link click on an AMP page. [Read more](journeys-via-amp.md#targeting-limitations).
- Only templates on the bottom of the page and equal to 76px in height show on AMP.
- [Dismiss period](https://help.branch.io/using-branch/docs/creatives-manager#dismissal-settings) is not supported - after dismiss, Journeys will show again during the next AMP session.
- [Client-side JavaScript controls](https://help.branch.io/using-branch/docs/journeys-faqs#client-side-javascript-controls-faqs) are not supported.
