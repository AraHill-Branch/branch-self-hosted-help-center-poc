...
::: info Tracking Web Campaigns with Self-Attributing Networks (SANs)
...
For tracking app campaigns, please see the relevant documentation for [Google](google-ads.md) and [Facebook](facebook-ads-overview.md).
...
First, you'll want to ensure you are capturing web conversions by [integrating the Branch Web SDK](web-sdk-overview.md) and the [relevant [conversion events](web-advanced-features.md) for web]. When the SDK is integrated, important metrics such as *web session starts* are automatically captured, and you should add additional conversion events as noted above.

### Creating links

::: tip Analytics Tag Values
...
1. Navigate to the relevant partner (e.g. Google) to [create an ad link](google-ads.md).
2. Ensure all the redirects are set to *websites* to avoid any policy violations.
3. **IMPORTANT**. Ensure that the link has the following properties:
   ...
   2. **Campaign ID** (*~campaign\_id* in link data). Campaign ID is required to avoid duplicating clicks. More on this in the troubleshooting section.

      1. For Google, the Campaign ID can be dynamically filled in using the `{campaignid}` ValueTrack Parameter *~campaign\_id={campaignid}*.  
...
2. If link clicks containing campaign ID are recorded, we *won't* show any click data *ingested via API* in the dashboard, to avoid double counting link clicks & API-ingested clicks. So, if you put links in some, but not all, ads in a campaign you can expect to see missing clicks.
3. If you have a link with a campaign name matching the ad network's campaign name, but *no campaign ID on the link*, you will see duplicate click numbers in the dashboard.
4. For link-tracked web campaigns, you can only compare by values that are present as an analytics tag on the link. So if you have a campaign ID, but don't have ~campaign or ~ad\_name etc. on the link, you can't compare by campaign name in the dashboard, and data might seem "missing."